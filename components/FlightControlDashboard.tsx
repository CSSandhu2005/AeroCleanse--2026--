...
const INITIAL_WAYPOINTS: Waypoint[] = [
  { id: "wp-a", label: "A", lat: 28.6139, lng: 77.209 },
  { id: "wp-b", label: "B", lat: 28.6155, lng: 77.212 },
  { id: "wp-c", label: "C", lat: 28.617, lng: 77.21 },
  { id: "wp-d", label: "D", lat: 28.618, lng: 77.2075 },
  { id: "wp-e", label: "E", lat: 28.616, lng: 77.2055 },
];

const MOCK_ALERTS: Alert[] = [
  {
    id: 1,
    message: "Low battery warning (22%)",
    level: "critical",
    time: "2m ago",
  },
  {
    id: 2,
    message: "High wind detected (18 km/h)",
    level: "warning",
    time: "5m ago",
  },
  {
    id: 3,
    message: "GPS signal weak in Zone 3",
    level: "warning",
    time: "8m ago",
  },
  {
    id: 4,
    message: "Mission completed — Field 5",
    level: "info",
    time: "12m ago",
  },
];
...
const NeonButton = ({
  ...
  const variants = {
    emerald: "bg-emerald-600 text-white hover:bg-emerald-500 shadow-none",
  ...
  return (
    <GlassCard glow className="flex flex-col gap-4 h-full">
      ...
      <div className="space-y-2">
        ...
        <NeonButton variant="emerald" className="w-full text-center">
...
const TelemetryPanel = () => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setSpeed((v) =>
        Math.max(0, +(v + (Math.random() - 0.5) * 0.5).toFixed(1)),
      );
      setSignal(Math.floor(Math.random() * 2) + 3);
      setWind((v) =>
        Math.max(0, +(v + (Math.random() - 0.5) * 1.5).toFixed(1)),
      );
      setTick((t) => t + 1);
    }, 2000);
  ...
  useEffect(() => {
    if (tick % 10 === 0 && tick > 0)
      setModeIdx((i) => (i + 1) % DRONE_MODES.length);
  }, [tick]);
      ...
      {[1, 2, 3, 4, 5].map((bar) => (
        <div
          ...
          className={`w-2 rounded-sm transition-all duration-500 ${
            bar <= strength ? "bg-emerald-500" : "bg-slate-800"
          }`}
  ...
  return (
    <GlassCard glow className="flex flex-col gap-3 h-full">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-[11px] font-bold px-2 py-0.5 rounded-full border ${modeColor}`}
        >
          {mode}
        </span>
      </div>
      ...
      <div className="bg-slate-800/50 border border-slate-800 rounded-xl p-3">
        <div className="flex justify-between text-xs mb-2">
          ...
          <span
            className={`font-mono font-bold ${battery < 20 ? "text-red-400" : "text-emerald-500"}`}
          >
            {battery.toFixed(1)}%
          </span>
        </div>
      ...
      <div className="space-y-2">
        {MOCK_ALERTS.map((alert) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-xs font-medium ${alertTextStyle[alert.level]}`}
              >
                {alert.message}
              </p>
