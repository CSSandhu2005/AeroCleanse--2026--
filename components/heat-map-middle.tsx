...
import {
  ...
  SequentialLegend,
  schemes, // Assuming 'schemes' is correctly imported for colorScheme
} from 'reaviz';
...
interface HeatmapDataPoint {
  key: string; // Y-axis category (e.g., Day of Week)
  data: Array<{
    key: string; // X-axis category (e.g., Hour of Day)
    data: number | null; // Heatmap value
  }>;
}

const heatmapSmallSimpleBlocksDataRaw: HeatmapDataPoint[] = [
  {
    key: 'Mon',
    data: [
      { key: '00:00', data: 10 }, { key: '03:00', data: 15 }, { key: '06:00', data: 20 },
      { key: '09:00', data: 25 }, { key: '12:00', data: 30 }, { key: '15:00', data: 22 },
      { key: '18:00', data: 18 }, { key: '21:00', data: 12 },
    ],
  },
  {
    key: 'Tue',
    data: [
      { key: '00:00', data: 5 }, { key: '03:00', data: 10 }, { key: '06:00', data: 18 },
      { key: '09:00', data: 30 }, { key: '12:00', data: 35 }, { key: '15:00', data: 28 },
      { key: '18:00', data: 20 }, { key: '21:00', data: 15 },
    ],
  },
  {
    key: 'Wed',
    data: [
      { key: '00:00', data: 8 }, { key: '03:00', data: 12 }, { key: '06:00', data: 22 },
      { key: '09:00', data: 28 }, { key: '12:00', data: 32 }, { key: '15:00', data: 25 },
      { key: '18:00', data: 19 }, { key: '21:00', data: 10 },
    ],
  },
    {
    key: 'Thu',
    data: [
      { key: '00:00', data: 12 }, { key: '03:00', data: 18 }, { key: '06:00', data: 25 },
      { key: '09:00', data: 32 }, { key: '12:00', data: 38 }, { key: '15:00', data: 30 },
      { key: '18:00', data: 22 }, { key: '21:00', data: 16 },
    ],
  },
  {
    key: 'Fri',
    data: [
      { key: '00:00', data: 15 }, { key: '03:00', data: 20 }, { key: '06:00', data: 28 },
      { key: '09:00', data: 35 }, { key: '12:00', data: 40 }, { key: '15:00', data: 33 },
      { key: '18:00', data: 25 }, { key: '21:00', data: 18 },
    ],
  }
];

// Validate and prepare chart data
const validatedHeatmapData = heatmapSmallSimpleBlocksDataRaw.map(yCategory => ({
  ...yCategory,
  data: yCategory.data.map(xCategory => ({
    ...xCategory,
    data: (typeof xCategory.data === 'number' && !isNaN(xCategory.data)) ? xCategory.data : 0,
  })),
}));

const heatmapColorScheme = [ // Array of objects with fill and filter
  { fill: '#FFD440', filter: 'drop-shadow(0px 0px 5px #FFD44090)' },
  { fill: '#F8A340' },
  { fill: '#E84045' },
  { fill: '#BB015A' }, // Added another color for more range
  { fill: '#8A0042' }, // Added another color for more range
];
...
interface HeatmapIncidentReportCardProps {
  title?: string;
}

function HeatmapIncidentReportCard({ title = "Incident Report" }: HeatmapIncidentReportCardProps): React.ReactNode {
  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-[375px] h-[560px] overflow-hidden transition-colors duration-300">
      <h3 className="text-3xl text-left p-7 pt-6 pb-8 font-bold text-neutral-800 dark:text-white">
        {title}
      </h3>
      <div className="flex w-full h-[240px] pl-2 pr-2"> {/* Adjusted height for heatmap + legend */}
        <Heatmap
          height={240}
          data={validatedHeatmapData}
          yAxis={
            <LinearYAxis
              type="category"
              axisLine={null}
              tickSeries={
                <LinearYAxisTickSeries
                  line={null}
                  label={<LinearYAxisTickLabel padding={10} fill="currentColor" className="text-neutral-600 dark:text-neutral-400" />}
                />
              }
            />
          }
          xAxis={
            <LinearXAxis
              ...
              tickSeries={
                <LinearXAxisTickSeries
                  line={null}
                  label={<LinearXAxisTickLabel padding={10} rotation={-60} fill="currentColor" className="text-neutral-600 dark:text-neutral-400" />}
        ...
        <SequentialLegend
          data={validatedHeatmapData}
          colorScheme={schemes.unifyvizWarm} // Using a predefined scheme from reaviz/schemes
          gradientClassName="!w-[20px]"
          className="pl-1 pr-1 mt-8 !h-[130px] text-neutral-600 dark:text-neutral-400"
        />
      </div>
      <div className="flex flex-col pl-8 pr-8 pt-6 font-mono divide-y divide-neutral-200 dark:divide-[#262631]">
        {metrics.map(metric => (
          <motion.div
            key={metric.id}
...
export default HeatmapIncidentReportCard;